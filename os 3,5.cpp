...
void worstFit(struct MemoryBlock memory[], int numBlocks, int requestSize)
{
    ...
    if (worstFitIndex != -1) 
	{
        ...
        printf("Allocated %d units of memory to Process %d using Worst Fit.\n", requestSize, memory[worstFitIndex].id);
    } 
	else 
	{
        printf("Memory allocation failed for Process. No suitable block found using Worst Fit.\n");
    }
}
void displayMemory(struct MemoryBlock memory[], int numBlocks) 
{
